...
| `evidence_code` | text | yes | AIM evidence code. | Generated or validated by AIM backend. |
...
| `expected_checksum_sha256` | text | no | Optional declared checksum. | If present, completion must verify the checksum or block. |
...
- If an expected SHA-256 checksum is declared, completion must verify it through the provided checksum or object metadata; otherwise completion is blocked.
