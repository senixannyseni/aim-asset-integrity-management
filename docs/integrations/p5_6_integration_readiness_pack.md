...
Do not paste secrets, JWTs, passwords, object-storage keys, signed URLs, production credentials, database connection strings with passwords, private keys, confidential client evidence, raw database dumps, webhook secrets, CMMS credentials, notification tokens, or vulnerability exploit details into P5-6 documents. Use redacted fixtures and attach sensitive evidence only in approved secure evidence storage.
