...
| SEC-002 | Security risk | Malicious or unsafe file is uploaded as evidence. | Upload validation only checks extension; no MIME/checksum/scanning control. | Malware exposure or unsafe file distribution to users. | 3 | 4 | 12 | Validate extension, MIME, file size, checksum, and integrate malware scanning placeholder before production. | Quarantine file; block preview/download; investigate uploader and affected objects. | IT Admin | Open | 2026-06-30 |
