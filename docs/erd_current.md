...
AIM/PostgreSQL stores final structured engineering data, metadata, validation snapshots, Formula Registry metadata, workflow events, error logs, and audit logs. n8n may create workflow events and error logs through AIM APIs only. Universal deterministic calculation execution is included through Sprint 6. FFS trigger workflow governance is included through Sprint 7. RBI interface workflow governance is included through Sprint 8. Subsequent governance phases now include AI extraction/staging controls, report generation/issue gates, internal AIM work-order fallback, and RC3-B object-storage evidence/report artifact metadata. The repository still excludes proprietary API/API-ASME formula expression execution, full API 579/API 581 quantitative assessment, external CMMS integration, and 3D processing.
...
No API/API-ASME formulas, full API 579/API 581 quantitative calculation, external CMMS integration, or 3D processing are implemented in this sprint. Later governance phases in this repository now implement AI extraction/staging controls, report generation/issue gates, and internal AIM work order fallback while AIM remains the system of record and n8n remains API-only orchestration.
...
## RC3-B Closeout Polish Addendum

RC3-B closeout tightens the object-storage evidence model to match the source-of-truth evidence governance rules:

- Evidence IDs for gate-eligible object-storage uploads are generated by the AIM backend using the `EVD-{YYYY}-{six_digit_number}` convention; caller-supplied evidence codes are not used for the upload-session flow.
- Gate-eligible evidence upload sessions require a SHA-256 checksum before a signed upload URL is issued.
